...
class ForeignEmploymentApplication
{
    ...
    {
        if ($id) {
            $query = "SELECT * FROM foreign_employment_applicant WHERE id=" . $id;
            $db = new Database();
    ...
    {
        $query = "INSERT INTO foreign_employment_applicant (full_name, staff_id, nic, passport_number, birthday, age, gender, marital_status, mobile_number, whatsapp_number, province_id, current_job, job_abroad, type, created_at) VALUES ('"
            . $this->full_name . "', '"
            . $this->staff_id . "', '"
            . $this->nic . "', '"
    ...
    {
        $query = "UPDATE foreign_employment_applicant SET "
    ...
    {
        $query = "DELETE FROM foreign_employment_applicant WHERE id='" . $this->id . "'";
        $db = new Database();
        return $db->readQuery($query);
    }
    ...
    public function all()
    {
        $query = "SELECT * FROM foreign_employment_applicant ORDER BY created_at DESC";
        $db = new Database();
        $result = $db->readQuery($query);
    ...
    {
        $query = "SELECT * FROM foreign_employment_applicant WHERE staff_id = 0 ORDER BY created_at DESC";
        $db = new Database();
        $result = $db->readQuery($query);
    ...
    {
        $db = new Database();
        $query = "SELECT * FROM foreign_employment_applicant WHERE staff_id = $staff_id ";
