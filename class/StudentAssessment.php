...
class StudentAssessment {
    ...
    public function __construct($id = null) {
        if ($id) {
            ...
            if ($result) {
                $this->id = $result['id'];
                $this->agancy_student_id = $result['agancy_student_id'];
    ...
    public function create() {
        ...
        $query = "INSERT INTO `student_assessment` (
            ...
            . ($this->agancy_student_id !== null ? "'" . mysqli_real_escape_string($con, $this->agancy_student_id) . "'" : "NULL") . ", "
        ...
        if ($result) {
            ...
            error_log("Created assessment with ID: " . $this->id . " for agancy_student_id: " . ($this->agancy_student_id ?? 'NULL'));
            return $this->id;
        } else {
    ...
    public function update() {
        ...
        $query = "UPDATE `student_assessment` SET 
            `agancy_student_id` = " . ($this->agancy_student_id !== null ? "'" . mysqli_real_escape_string($con, $this->agancy_student_id) . "'" : "NULL") . ",
    ...
    public function getByStudentAndType($agancy_student_id, $type) {
        if (empty($agancy_student_id)) {
            return false;
        }

        $query = "SELECT * FROM `student_assessment`
                 WHERE `agancy_student_id` = " . intval($agancy_student_id) . "
    ...
    public function getAllByStudent($agancy_student_id) {
        if (empty($agancy_student_id)) {
            return [];
        }
    ...
    public function saveOrUpdate($agancy_student_id, $type, $date, $result) {
        // Set the agency student ID directly
        $this->agancy_student_id = $agancy_student_id;
        ...
        // Create new record (allow unlimited assessments for same student)
        $newId = $this->create();
        if ($newId) {
            error_log("Created new assessment ID: {$newId} for agancy_student_id: {$agancy_student_id}");
            return $newId;
        }
    ...
    public function deleteNullStudentRecords() {
        $db = new Database();
        $query = "DELETE FROM `student_assessment` WHERE `agancy_student_id` IS NULL";
        $result = $db->readQuery($query);
        
        if ($result) {
            $deletedCount = mysqli_affected_rows($db->DB_CON);
            error_log("Deleted {$deletedCount} assessment records with NULL agancy_student_id");
            return $deletedCount;
        }
